...
- **Компенсации, привязанные к жизненному циклу** (`AddCleanup`): освобождение регистрируется рядом с
...
`Manager` создаёт общий `AppContext` (`Registry` и логгер) и внедряет его в
каждый модуль, реализующий `ContextAware`. `BaseAppModule` уже реализует этот
интерфейс, поэтому встраивающий модуль получает общие сервисы через
`m.AppContext()`. Снаружи тот же экземпляр доступен через `Manager.Registry()`.

**Registry.** Модуль *предоставляет* реализацию
...
Всё, что нужно освободить при остановке модуля, регистрируйте рядом с местом
захвата через `AddCleanup`; очистки выполняются в обратном порядке регистрации
на `Destroy` и при откате неудачного `Init`.

### Уведомления

Собственной шины событий в пакете нет, и это осознанно. Ответить на запрос и
оповестить о факте — разные механизмы с разными режимами отказа: один
возвращает ошибку вызывающему, другой ставит в очередь и может потерять.
Спрятать оба за одним интерфейсом — значит сделать поведение вызова зависящим от
того, какую реализацию подключили при сборке приложения.
...
поддерживает этот проект: типизированные топики, доставка через очередь, явные
политики перегрузки. Модуль [`adapters/hubmod`](adapters/hubmod) связывает их:
он владеет хабом как модулем и привязывает подписки к жизненному циклу модуля,
чтобы перезапущенный модуль не оказался подписан дважды. Это отдельный
Go-модуль, поэтому сам appmod остаётся без этой зависимости.
...
| [`manager`](examples/manager) | Оркестрация графа модулей через `Manager` и общение модулей: доступ к данным `api → cache → db` через `Provide`/`Require`, плюс наблюдение за остановкой в фоновом воркере. |
...
У адаптеров свои примеры: им нужна зависимость, которой в этом модуле нет. Они
лежат в отдельном Go-модуле, поэтому запускаются из каталога адаптера, а не из
корня репозитория:

| Пример | Что демонстрирует |
| --- | --- |
| [`hubmod/orders`](adapters/hubmod/examples/orders) | Приложение на шине событий: `hubmod.NewModule` владеет хабом, `Require` берёт его из `Registry` в стартовом хуке, `SubscribeModule` привязывает подписку к модулю, синхронная и очередная доставка рядом на одном топике, перезапуск модуля посреди прогона без двойной подписки и остановка, закрывающая хаб после всех издателей. |
...
| `appcontext.go` | Общий `AppContext` (`Registry` + логгер + широковещание остановки) и возможность `ContextAware`.        |

Рядом с пакетом лежит каталог [`adapters/`](adapters) — **отдельные Go-модули**, связывающие
appmod с другими библиотеками. У каждого свой `go.mod` и свои теги, поэтому набор зависимостей
самого appmod не меняется, а импорт appmod не тянет их зависимости.

| Модуль | С чем связывает |
|---|---|
| [`adapters/hubmod`](adapters/hubmod) | [`msghub`](https://github.com/efureev/msghub) — хаб как модуль с жизненным циклом и подписки, привязанные к модулю. |
