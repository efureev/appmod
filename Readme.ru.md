...
и переходит в `StateFailed`. Ошибки отката объединяются с исходной
...
> **Teardown-хуки обязаны быть nil-безопасными и идемпотентными.** Откат
> безусловен: пакет не связывает стартовый хук с конкретным teardown-хуком и
> потому не может знать, какие компенсации уместны. Выполняются все teardown-хуки
> — даже если упал самый первый `BeforeStart` и ничего не было захвачено, так что
> хук, слепо вызывающий `pool.Close()`, разыменует nil. Пропускать teardown тоже
> нельзя: утечёт всё, что успели захватить отработавшие стартовые хуки. Защищайте
> teardown-хуки соответственно.
