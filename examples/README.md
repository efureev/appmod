...
| [`manager`](./manager) | Orchestrating a dependency graph with `Manager`, plus inter-module communication via the shared `Registry` (`db` provides `DB`, `cache` requires `DB` and provides `Cache`, `api` requires both — `api → cache → db`), and a `worker` observing the shutdown, all sharing the `AppContext` the `Manager` injects. |
